...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/QubixParams"
else
    PARAMS_DIR="$HOME/.qubix-params"
fi
...
DOWNLOAD_URL="https://github.com/Qubix-Core/qubix/releases/tag/1.3.17.03"
...
# fetch methods can be disabled with QXB_DISABLE_SOMETHING=1
QXB_DISABLE_WGET="${QXB_DISABLE_WGET:-}"
#QXB_DISABLE_IPFS="${QXB_DISABLE_IPFS:-}"
QXB_DISABLE_CURL="${QXB_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$QXB_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$QXB_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$QXB_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Qubix parameters!
...
This script will fetch the Qubix SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Qubix SNARK parameters. Note that it is
